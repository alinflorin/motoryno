...
4. **Manual overrides** (the car's _OBD setup_ screen, `src/app/car/[carId]/obd.tsx`, model in `src/obd/manualConfig.ts`): for cars none of the above fits, the user can set extra adapter init commands (`ObdConfig.initCommands`, e.g. `ATSP6`), a custom VIN request (`vinSource`) and a custom odometer request with its byte field (`odometerSource`) - protocol, `ATSH` header, `ATCRA` reply ID, session request, read request, offset/length/byte order/scale - and test each one live against the car before saving. Whatever is saved is tried first on every connection; the built-in strategies remain the fallback.
