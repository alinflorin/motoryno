 ...
 *   into range. Replays the car's learned `odometerSource` when it has one
...
/**
 * Reads the VIN via the standard Mode 09 request, falling back to the
 * manufacturer UDS DID (Mode 22, F190) aimed at each known ECU header in
 ...
 * this just works through every header the bundled catalogs use.
 */
...
async function runOdometerSync(device: Device, car: OdometerSyncTarget): Promise<OdometerSyncResult> {
  ...
  try {
    ...
    if (car.odometerSource) {
      const learned = await readFirstOdometer(connection, [car.odometerSource]);
      if (learned) return { odometerKm: learned.km, odometerSource: null, connectionFailed: false };
      obdLog('info', `persisted source ${car.odometerSource.label} gave no reading - falling back to known requests`);
    }
 ...
 */
export function scanVehicleInfo(device: Device, onStep?: (step: ScanStep) => void): Promise<VehicleScanResult> {
  return withAdapterLock(device.id, () => runVehicleScan(device, onStep));
}

async function runVehicleScan(device: Device, onStep?: (step: ScanStep) => void): Promise<VehicleScanResult> {
  ...
  try {
    ...
    const odometer = await readFirstOdometer(connection, vehicleOdometerSources(result.vin, result.make));
